...
const degreesRotated = new Map<Direction, any>(directions.map((direction, index) => {
    const degrees = index * 90;
    return [direction, {transform: [{rotate: `${degrees}deg`}] }];
}));

const AdvanceButton: React.FC<IProps> = ({onPress, direction = 'right', active = false, width = 50}) => {
