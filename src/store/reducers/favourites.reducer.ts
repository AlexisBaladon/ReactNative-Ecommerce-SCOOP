...
const favouritesReducer = (state: FavouritesState = initialState,  action: FavouritesActions): FavouritesState => {
    let item: DtItem | undefined;
    if (action.itemId !== undefined) { item = items.find(item => item.id === action.itemId) };
    switch (action.type) {
